...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-20 border-t border-[#E5E7EB] bg-white/95 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur">
      <div className="mx-auto flex w-full max-w-[540px] items-center justify-around px-6">
        {items.map((item) => {
          const active = router.pathname === item.href;

          return (
            <Link key={item.href} href={item.href} className="flex min-w-[72px] flex-col items-center gap-1 py-1">
              <img
